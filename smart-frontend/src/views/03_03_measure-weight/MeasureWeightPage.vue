...
const nextItem = () => {
  ...
  router.push('/update/weight/barcode-scan')
}

const handleConfirm = () => {
  measureStore.updateEditingItemMeasuredAt()
  router.push('/update/weight/confirm')
}
</script>

<template>
  <Title text="重量測定" class="mb-4">
    <BackButton to="/update/weight/barcode-scan" />
  </Title>
  ...
  <Footer>
    ...
    <FooterButton position="2" variant="secondary" @click="router.push('/update/weight/list')">
      測定一覧
    </FooterButton>
